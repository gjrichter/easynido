...
var fEmbedAudioInInfo = false;
var fShowTooltips = false;
var fScrollFixListHeader = false;
...
ixmaps.MapUp = MapUp = function(map,itemDiv,legendDiv) {
	...
	/** holds an optional marker clusterer */
	this.markerDeclutter = (typeof(MarkerDeclutter)!="undefined")?new MarkerDeclutter(map,null,{gridSize:50}):null;
...
MapUp.prototype.addLayer = function(szSourceName,source) {
	...
	for ( var i=0; i<layerCount; i++ ){
        ...
        //
		if ( layer.link ){
			tempLayerA.push({master:source.szSource,source:szSourceName,name:layer.properties.name,data:layer,icon:defaultIcon,bbox:bBox,upper:nUpper,lower:nLower});
		}
        else
		...
		if ( layer.features ){
			...
			var isLayeri = this.getLayerIndex(szSourceName,layer.properties.name);
			if ( (isLayeri >= 0) && this.layer[isLayeri].data.link ){
				// yes, then replace it
				this.layer[isLayeri] = {source:szSourceName,title:szTitle,name:layer.properties.name,data:layer,icon:defaultIcon,bbox:bBox,upper:nUpper,lower:nLower};
			}else{
				// no, append new layer
				tempLayerA.push({source:szSourceName,title:szTitle,name:layer.properties.name,data:layer,icon:defaultIcon,bbox:bBox,upper:nUpper,lower:nLower});
			}
		}
		else
		...
		if ( layer.layers ){
			...
			for ( var ii=0; ii<subLayerCount; ii++ ){
				...
				if ( sublayer && sublayer.features ){
					...
					// detect if there is a placeholder for the loaded layer
					var isLayeri = this.getLayerIndex(szSourceName,sublayer.properties.name);
					if ( (isLayeri >= 0) && this.layer[isLayeri].data.link ){
						// yes, then replace it
						this.layer[isLayeri] = {source:szSourceName,parent:layer,name:sublayer.properties.name,data:sublayer,icon:defaultIcon,bbox:bBox,upper:nUpper,lower:nLower};
					}else{
						// no, then append it
						sublayer.properties.name = this.makeUniqueLayerName(sublayer.properties.name);
						tempLayerA.push({source:szSourceName,title:szTitle,parent:layer,name:sublayer.properties.name,data:sublayer,icon:defaultIcon,bbox:bBox,upper:nUpper,lower:nLower});
					}
...
/**
 * retirns if layer exists by its name and origin
 * @param szName the layer or sublayer name
...
MapUp.prototype.setExtent = function(szMode,szLayer) {
	...
	if ( (szMode == "tolayer") && szLayer && this.data ){
		...
		this.actualMapCenter = this.map.getCenter();
		this.actualMapZoom = this.map.getZoom();
	...
	if ( (szMode == "actual") ){
		if ( this.actualMapCenter ){
			_map_setCenterAndZoom(map,this.actualMapCenter,this.actualMapZoom);
			return;
		}else
	...
	if ( this.actualMapCenter && this.actualMapZoom) {
		this.map.setCenter(this.actualMapCenter, this.actualMapZoom);
	}
...
MapUp.prototype.changeZoom = function(nDelta) {
	...
	this.map.setZoom(this.actualMapZoom+nDelta);
	this.actualMapZoom = this.map.getZoom();
};
/**
...
MapUp.prototype.drawSidebar = function() {
	...
	for ( var i=0; i<this.layer.length; i++ ){
		...
		nLayer += this.makeItemList(this.layer[i].data,this.layer[i].defaultIcon,this.layer[i]);
	}

	if ( !nLayer ){
		var paragraph = document.createElement("p");
		paragraph.innerHTML = "( lista vuota )";
	...
	if ( !this.fUseListStateButtons && ($("#sidebar-footer")[0]) ){
		...
		// szHTML += "<a href=\"javascript:ixmaps.jsapi.removeData('"+"all"+"')\"><img src='resources/ui/delete.png' height='22' style=\"opacity:0.2;float:right;margin-right:11px;margin-top:5px\" /></a>";
		szHTML += "</div>";
...
MapUp.prototype.synchronize = function() {
	...
	if ( this.fClipSidebarToMapExtent || ixmaps.jsapi.fMarkerClick ){
		ixmaps.jsapi.fMarkerClick = false;
		this.clearSidebar();
		this.drawSidebar();
	}else{
...
/**
 * synchronize google map after resize
 */
MapUp.prototype.checkResize = function() {
	if (this.map){
 ...
 */
MapUp.prototype.makeItemList = function(layer,defaultIcon,layerObj) {
	...
	if ( 1 ){
		...
		if ( (this.lastSource != layer.source) ){
			...
			var szName = layer.properties.name;
			if ( layer.parent && layer.parent.properties.name ){
				szName = layer.parent.properties.name;
			}
			if ( layerObj.title ){
				szName = layerObj.title;
			}
			szName = layer.source;

			var szHTML = "";
				if ( layer.legend == "collapsed" ){
					szHTML += "<a style=\'padding:0\' href=\'javascript:ixmaps.jsapi.setListContent(\""+szName+"\",\"show\");\'>";
					szHTML += "<p class=\"title\" >" + (layerObj.title || szName);
					szHTML += "<img src='resources/ui/collapsed.png' height='22' style=\"float:right;vertical-align:-8px;margin-left:20px;\" />";
				}else{
					szHTML += "<a style=\'padding:0\' href=\'javascript:ixmaps.jsapi.setListContent(\""+szName+"\",\"hide\");\'>";
					szHTML += "<p class=\"title\" >" + (layerObj.title || szName);
					szHTML += "<img src='resources/ui/expanded.png' height='22' style=\"float:right;vertical-align:-8px\" />";
				}
//				szHTML += "<a href=\"javascript:ixmaps.jsapi.removeData('"+layer.source+"')\"><img src='resources/ui/delete.png' height='22' style=\"float:right;vertical-align:-2px\" /></a>";
				szHTML += "</p>";
				szHTML += "</a>";
	...
	if ( layer.parent && layer.parent.legend == "collapsed" ){
	...
	if ( 1 ){
		...
		if ( (layer.properties.name != layer.source)){
			...
			if ( layer.link ){
				...
				var szLayerSource = __getUrlRoot(layerObj.master) + layer.link.properties.href + (layerObj.master.match(/.gz/)?".gz":"");
				listItem.innerHTML +=  "<span style=\"margin-left:0px;\"></pan>";
				listItem.innerHTML += "<a class=\"listitemname\" href=\'javascript:ixmaps.szMasterSource=\""+layer.source+"\";ixmaps.szLinkLayer=\""+layer.properties.name+"\";ixmaps.jsapi.addData(\""+szLayer+"\",\"\",\""+szLayerSource+"\");\'><img src='resources/ui/box.gif' height='14' style=\"vertical-align:-5px\" /></a>" ;
			}		
			else
	...
	if ( 1 ){
		...
		if ( layerObj.upper && (layerObj.upper < this.actualMapScale) ){
			...
			if ( (layer.fUseListStateButtons || (layer.parent && layer.parent.fUseListStateButtons )) && !(layer.legend == "collapsed") ){
				var szHTML = "";
				szHTML += "<div class=\"listitemcontrolgroup\" >";
				szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"none\");\'><img src=\"resources/ui/listview.jpg\" height=\"32\" style='opacity:"+(!layer.fShowInfoInList&&!layer.properties.fGallery?0.2:0.2)+";border:"+(!layer.fShowInfoInList&&!layer.properties.fGallery?"#666 solid 1px":"0")+";'></a>";
				szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"info\");\'><img src=\"resources/ui/articelview.jpg\" height=\"32\" style='opacity:"+(layer.fShowInfoInList?0.2:0.2)+";border:"+(layer.fShowInfoInList?"#666 solid 1px":"0")+";'></a>";
				szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"gallery\");\'><img src=\"resources/ui/galleryview.jpg\" height=\"32\" style='opacity:"+(layer.properties.fGallery?0.2:0.2)+";border:"+(layer.properties.fGallery?"#666 solid 1px":"0")+";'></a>";
	...
	if ( layer.properties.fGallery && !layer.fShowInfoInList ){
		this.makeItemListGallery(layer,defaultIcon,markers);	
	}else{
		this.makeItemListItems(layer,defaultIcon,markers);
	}
 ...
 * @param defaultIcon a default icon image for the marker
 */
MapUp.prototype.makeItemListItems = function(layer,defaultIcon,markers) {
	...
	if ( (_mapup_explDate == "") && this.timeline && layer.duration ){
		var d = new Date(this.timeline.lastTime);
	...
	for(var i=0; i < markers.length && (nItems < maxItems); i++) {
		...
		// get icon object 
		var icon = _mapup_getIcon(this.map,defaultIcon,layer,markers,i,'nosmall');
		...
		if ( this.isQuery() && !this.isMarkerInSearchQuery(markers[i]) ){
			fVisible = false;
		}
		else
		if ( !this.isMarkerInFilter(layer,markers[i]) ){
			fVisible = false;
		}
		else {
			fVisible = true;
		...
		if ( markers[i].inLegend && !markers[i].fSuppressInfo ){
			...
			if ( this.map ){
				if ( markers[i].gOverlayObject && !markers[i].fSuppressInfo ){
					...
					handler = _mapup_createMarkerClickHandlerGoto(this.map, markers[i].gOverlayObject, layer, markers[i], layer.fShowInfoInList?"small":"normal");
				}else{
			...
			if ( !layer.fShowInfoInList ){
				...
				// --------------------------------------------------------------
				var d = new Date(markers[i].properties.utime);
				var szDate = szDayA[(d.getDay())] + " " + d.getDate()+" " +szMonthA[(d.getMonth())]+ " " +d.getFullYear();
				var szDateEnd = "";
				if ( markers[i].properties.utimeEnd ){
					var d = new Date(markers[i].properties.utimeEnd);
					var szDateEnd = " - "+szDayA[(d.getDay())] + " " + d.getDate()+" " +szMonthA[(d.getMonth())]+ " " +d.getFullYear();
				}
			...
			if ( __getType(markers[i]) == "LineString" ){
				...
				szRowContent += "<img src=\""+myIcon+"\" height=\"26\" />";
			}
			//szRowContent += "</a>";
			...
			if ( markers[i].gOverlayObject) {
				...
				szRowContent += "&nbsp;  <a class=\"noprint\" title=\"mostra punto ingrandito\" href=\'javascript:ixmaps.jsapi.zoomTo("+center.lat()+","+center.lng()+");\'><img src='resources/ui/zoomto.png' height='22' style='float:right;margin-right:5px;' /></a>" ;
				if ( fFullScreen ){
					szRowContent += "<br/>";
				}
			}
			if ( layer.fShowInfoInList && markers[i].properties.description && !markers[i].fSuppressInfo ){
				...
				try	{
					szDesc = ixmaps.jsapi.onOpenInfoWindow(markers[i].properties.description,markers[i],"map");
				}
				catch (e){}
				...
				if ( markers[i].properties.data.INDIRIZZO ){
					szRowContent += " - "+markers[i].properties.data.INDIRIZZO;
				}
				if ( markers[i].properties.data.TELEFONO ){
					szRowContent += " - "+markers[i].properties.data.TELEFONO; 
				}
	...
	if ( nItems >= maxItems ){
		...
		$("<a href=\'javascript:ixmaps.jsapi.zoomToLayer(\""+layer.properties.name+"\");\' >&nbsp;&nbsp;- tutti "+(markers.length-nItems)+" elementi fuori quadro<img src='resources/ui/zoomto.png' style='margin-left:10px;vertical-align:-2px;height:14px'/></a>").appendTo(listItem); 
		listTable.appendChild(listItem);
	}
		...
		listItem.setAttribute("class","listitem");
		//$("<a href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"piu\");\' >&nbsp;&nbsp;- tutti "+(markers.length-nItems)+" elementi fuori quadro</a>").appendTo(listItem); 
		$("<a href=\'javascript:ixmaps.jsapi.zoomToLayer(\""+layer.properties.name+"\");\' ><br>&nbsp;&nbsp;"+(markers.length-nItems)+" elementi fuori quadro<img src='resources/ui/zoomto.png' style='margin-left:10px;vertical-align:-2px;height:14px'/></a>").appendTo(listItem); 
		listTable.appendChild(listItem);
	}
 ...
 * @param defaultIcon a default icon image for the marker
 */
MapUp.prototype.makeItemListGallery = function(layer,defaultIcon,markers) {
	...
	for(var i=0; i < markers.length && (nItems < maxItems); i++) {
		...
		if ( markers[i].inLegend && !markers[i].fSuppressInfo ){
			...
			if ( this.map ){
				if ( markers[i].gOverlayObject ){
					// if item visible, opem info window
					handler = _mapup_createMarkerClickHandlerGoto(this.map, markers[i].gOverlayObject, layer, markers[i], "normal");
				}else{
					// if item outside, goto 
					handler = _mapup_createGoToHandler(this.map, markers[i]);
				}
            ...
            var szListImage = "";
			if ( (!layer.duration || (markers.length < 50) || (szDate == _mapup_explDate)) && markers[i].properties.description.match(/\<img/i) ){
				var match = markers[i].properties.description.match(/src=(?:\"|\')?([^>]*[^/].(?:jpg|jpeg|bmp|gif|png))(?:\"|\')?/i);
				var szImg = (match && (match.length>1))?(match[1]):"";
				szListImage = "<a href=\"javascript:void()\"><img src="+szImg+" height='70' style='margin-right:3px' /></a>";
			}else{
...
function _mapup_getIcon(map,defaultIcon,layer,markers,i,szFlag) {	
		...
		if ( layerP && layerP.markerSizeRule ){
			...
			if ( match ){
				nSize = parseFloat(match[mSR.index]) / mSR.normalSize;
			}
		}
		...
		if ( layerP && (layerP.markerType == "custom") ){
			if ( layerP.smallImage && (!szFlag || !szFlag.match(/nosmall/)) ){
		...
		if (  markers[i].properties.icon || markers[i].properties.smallicon ){
			...
			if ( markers[i].properties.smallicon && fSmallIcon ){
				...
				icon.iconSize = new GSize(32*nScale, 32*nScale);
 ...
 * @param defaultIcon a default icon image for the marker
 */
MapUp.prototype.makeLayer = function(map,layer,defaultIcon) {	
	...
	if (!layer.duration && markers[0].properties.utime){
		layer.lastTime  = layer.firstTime = markers[0].properties.utime;	
		for ( var i=0; i<markers.length; i++ ){
	...
	if (1){
		for( var i=0; i < markers.length; i++ ) {
			if ( this.isMarkerInMapBounds(map,markers[i],0) &&
				 this.isMarkerInSearchQuery(markers[i])		&&
	...
	for( var i=0; i < markers.length; i++ ) {
		...
		this.makeOverlay(map,layer,markers,i,defaultIcon,layerObj.nMarkerCount<__nLargeIconMaxCount?'nosmall':null);
...
//				polygon.myPosition = new GLatLng(geometry.coordinates[0][1], geometry.coordinates[0][0]);
		...
		// -------------------------------
		if ( (!layer.properties.events || layer.properties.events.match(/ONCLICK/)) ){
			var handler = _mapup_createMarkerClickHandler(map, polygon, layer, markers[i],"normal");
		...
		// -----------------------------------------
		if ( (!layer.properties.events || layer.properties.events.match(/ONOVER/)) ){
	...
	// ---------------
		...
		var polyline = new GPolyline(polyPoints,markers[i].properties.style.lineStyle.color,Number(markers[i].properties.style.lineStyle.width),0.7);
		...
		// --------------------------------
		var handler = _mapup_createMarkerClickHandler(map, polyline, layer, markers[i],"normal");
	...
	// --------------
		...
		// get icon object 
		var icon = _mapup_getIcon(map,defaultIcon,layer,markers,i,szFlag);
		// make marker 
		...
		if ( markers[i].fVisible && !markers[i].fSuppressInfo ){
			var handler = _mapup_createMarkerClickHandler(map, marker, layer, markers[i],"normal");
...
function __getBounds(feature,i){
	...
	for ( var g=0; g<nCount; g++ ){
		...
		if ( geometry ){
			...
			for ( i=1; i<coordinatesA.length; i++ )	{
...
function __doGetPolygonCenterGLatLng(ptList){
	...
	for ( var a in ptList ){
		ptCenter.lat += ptList[a].lat();
		ptCenter.lng += ptList[a].lng();
	}
...
function _mapup_createMarkerClickHandlerGoto(map, marker, layer, info, szMode ) {
	return function() {
		...
		try {
			...
			ixmaps.jsapi.zoomTo(coordinates[1],coordinates[0]);
		}
		catch (e){}

		_mapup_createMarkerClickHandler(map, marker, layer, info, szMode + "|listitem" )();
	};
}
/**
 ...
 */
function _mapup_createMarkerClickHandler(map, marker, layer, info, szMode) {

	return function() {

		if ( !szMode.match(/listitem/) ){
			ixmaps.jsapi.fMarkerClick = true;
		}

		if ( actualInfoMarker == marker ){
			ixmaps.jsapi.fixMarkerClipping();
			var center = marker.getPoint();
			ixmaps.jsapi.zoomTo_step(center.lat(),center.lng());
			return false;
		}
		...
		// make info content
		//
		var szInfo = "<div class=\"InfoWindowHeader\">"+szTitle+"<\/div>";
		var szDesc = "";
		if ( fDescr || !szTitle.length || (info.properties.description.length < nClipDescription) ){
			szDesc += "<div class=\"InfoWindowBody\" " + (info.properties.description.match(/img/)?"style=\"min-height:200px;\"":"") + " >" + info.properties.description + "<\/div>";
		}else{
			if ( ixmaps.jsapi.mapParam && ixmaps.jsapi.mapParam.all && ixmaps.jsapi.mapParam.all.smallInfoKeepImage ){
				...
				szDesc += "<a class=\"more-link\" style=\"float:left;padding:3px;padding-bottom:0px;margin-top:-5px;margin-right:10px;\" href=\"javascript:ixmaps.jsapi.popupFullDescription();\"><img src='resources/ui/arrow_right.png' height='22' /></a>";
		...
		if ( fEmbedAudioInInfo ){
			szInfo += ixmaps.jsapi.getEmbedAudioHTML(ixmaps.jsapi.getResourceUrl(info.properties.data.LNK_2),250);
		}
 ...
 * @param defaultIcon a default icon immage for the marker
 */
MapUp.prototype.isMarkerInSearchQuery = function(marker) {
...
Timeline.prototype.showTimeWindow = function(){
	...
	**/
	...
	var szDate  = "<div style='min-width:150px;color:"+szColor+"'>";
...
MapUp.prototype.setTimeScope = function(szScope){
	if ( this.timeline ){
		...
		case "today":
			...
			this.timeline.high = d1.getTime() + 1000*60*60*24 - 1;
			days = 1;
			break;		
		case "day":
			days = 1;
