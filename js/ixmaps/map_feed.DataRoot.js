 ...
 */
	ixmaps.feed.parse.DataRoot = function(data,opt){
		...
		if ( opt.type == "json" ) {
			...
			console.log(data.query.results.dataroot);

			var szItem = null;

			for ( a in data.query.results.dataroot ){
				var szType = eval("typeof(data.query.results.dataroot."+a+")");
				if ( szType === "object" ){
					szItem = a;
				}
			}

			console.log("item = " + szItem);
			...
			var itemA = eval("data.query.results.dataroot."+szItem);
			...
			for ( i=0; i<itemA.length; i++ ){
				...
				if ( dateA.length && (dateA[0][2].length == 4) ){
					...
					feature.properties.utimeStart = d1;
				}
				...
				if ( dateA.length && (dateA[0][2].length == 4) ){
					var d1 =  new Date(dateA[0][2],dateA[0][1]-1,dateA[0][0]);
					feature.properties.utimeEnd = d1;
				}
